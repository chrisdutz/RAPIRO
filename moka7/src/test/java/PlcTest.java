...
public class PlcTest {
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        if (Connect())
        {
            ...
            while(true) {
                int Result = Client.ReadArea(S7.S7AreaPE, 0, 0, 1, Buffer);
                if(Result != 0) {
                    System.err.println("Error reading values.");
                    break;
                }
                String input = String.format("%8s", Integer.toBinaryString(Buffer[0])).replace(' ', '0');
                Result = Client.ReadArea(S7.S7AreaPA, 0, 0, 1, Buffer);
                if(Result != 0) {
                    System.err.println("Error reading values.");
                    break;
                }
                String output = String.format("%8s", Integer.toBinaryString(Buffer[0])).replace(' ', '0');
